...
import { IMAGE_QUALITY_THUMB } from "@/lib/media/config";
...
export const ActualitesSection = ({ title, items }: ActualitesSectionProps) => {
  return (
    <section className="site-section site-deferred-section" data-testid="public-home-actualites-section">
      <div className="site-container">
        ...
        <div className="site-news-grid" data-testid="public-home-actualites-grid">
          {items.map((article) => (
            <Link href={article.link as Route} prefetch={false} key={article.id} className="site-news-card" data-testid={`public-home-actualite-card-${article.id}`}>
              ...
              <div className="site-news-content">
                <div className="site-news-thumb">
                  <Image src={article.image} alt={article.title} width={96} height={96} sizes="96px" quality={IMAGE_QUALITY_THUMB} className="site-news-thumb-image" data-testid={`public-home-actualite-image-${article.id}`} />
                </div>
