...
import { PUBLIC_SITE_CACHE_TAGS } from "@/lib/cache/public-site";
...
const getHomePageContentByLocale = unstable_cache(
  ...
  {
    revalidate: 300,
    tags: [PUBLIC_SITE_CACHE_TAGS.home, PUBLIC_SITE_CACHE_TAGS.pages],
  },
);

export const getHomePageContent = cache(async () => {
  const locale = await getRequestLocale();
  return getHomePageContentByLocale(locale);
});