...
import {
  countDumpPagesByStatus,
  getDumpPageById,
  getDumpPageBySlug,
  isDumpFallbackEnabled,
  listDumpPages,
  listDumpPagesByIds,
} from "@/lib/data/dump-fallback";
...
export async function listPublishedPages(limit = 1000, locale?: string) {
  if (isDumpFallbackEnabled()) {
    return listDumpPages({ locale, status: "PUBLISHED", limit })
      .sort((left, right) => left.menuOrder - right.menuOrder || (right.updatedAt?.getTime() ?? 0) - (left.updatedAt?.getTime() ?? 0) || right.createdAt.getTime() - left.createdAt.getTime());
  }
...
export async function findPageBySlug(slug: string, locale: string) {
  if (isDumpFallbackEnabled()) {
    return getDumpPageBySlug(slug, locale);
  }
